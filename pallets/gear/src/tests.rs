...
fn mailbox_works() {
    ...
    new_test_ext().execute_with(|| {
        ...
        let (reply_to_id, _) = setup_mailbox_test_state(USER_1);
...
fn send_reply_works() {
    ...
    new_test_ext().execute_with(|| {
        // caution: runs to block 2
        let (reply_to_id, prog_id) = setup_mailbox_test_state(USER_1);
...
mod utils {
    ...
    pub(super) fn setup_mailbox_test_state(user: AccountId) -> (H256, H256) {
        let prog_id = {
